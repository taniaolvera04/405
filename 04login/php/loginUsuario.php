...
if($_POST){
    ...
    switch($action){
        case "registrar":
            ...
            $res=$cx->query($check);
            if($res->num_rows==0){
                $sql="INSERT INTO cecyusuario VALUES(null,'$a','$b','$c','$d',null)";
                if($cx->query($sql)){
                    $valido['success']=true;
        ...
        case "select":
            header('Content-Type: text/html; charset=utf-8');
                ...
                case "perfil":
                    header('Content-Type: text/html; charset=utf-8');
                        $valido['success']=array('success'=>false,'mensaje'=>"",'usuario'=>"",'password'=>"",'nombre'=>"",'apellidos'=>"",'foto'=>"");            
                        $a=$_POST['usuario'];
                        $check = "SELECT * FROM cecyusuario WHERE usuario='$a';";
                        $res=$cx->query($check);
                        if($res->num_rows>0){
                        ...
                        case "saveperfil":
                            header('Content-Type: text/html; charset=utf-8');
                                ...
                                $check="UPDATE cecyusuario SET nombre='$a',apellidos='$b',foto='$filePath' WHERE usuario='$c'";
                                $res=$cx->query($check);
                                if($res->num_rows>0){
                                    $row=$res->fetch_array();
                                    $valido['success']=true;
